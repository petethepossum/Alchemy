...
class PunishmentLookupCommands : BaseCommand()
{
    ...
    fun lookup(player: Player, @Name("punishmentId") id: String)
    {
        ...
        player.sendMessage(Chat.format("&7- &eType: &f" + punishment.getGrantable().niceName))
        ...
        {
            //string changed from Got Staff, to received staff rank
            player.sendMessage(Chat.format("&7- &eReceived Staff Rank: &f" + TimeUtil.formatDuration(System.currentTimeMillis() - profile.expirable.addedAt)))
        }
    }
}